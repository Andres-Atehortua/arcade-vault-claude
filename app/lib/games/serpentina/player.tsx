...
import { SerpentinaGame, type GameSnapshot } from './engine';

const WIDTH = 800;
const HEIGHT = 600;

/** Keys the game owns: their default scrolling is suppressed while mounted. */
const GAME_KEYS = ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'KeyP', 'Escape', 'Space'];
...
const sameSnapshot = (a: GameSnapshot, b: GameSnapshot) =>
  a.score === b.score && a.length === b.length && a.phase === b.phase;

/** While the alias field has focus its keys belong to the form, not to the snake. */
...
const SerpentinaPlayer = ({ game }: { game: GameRow }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const crtRef = useRef<HTMLDivElement>(null);
  const fullscreenToggleRef = useRef<(() => void) | null>(null);
  const gameRef = useRef<SerpentinaGame | null>(null);
  ...
  useEffect(() => {
    ...
    const instance = new SerpentinaGame(ctx, WIDTH, HEIGHT);
        ...
        <div className="crt-screen">
          ...
          <div className="touch-controls dpad">
            <div className="dpad-cross">
              <button className="touch-btn up" aria-label="Mover arriba" data-code="ArrowUp" {...touchHandlers}>
                ▲
              </button>
              <button className="touch-btn left" aria-label="Mover a la izquierda" data-code="ArrowLeft" {...touchHandlers}>
                ◀
              </button>
              <button className="touch-btn right" aria-label="Mover a la derecha" data-code="ArrowRight" {...touchHandlers}>
                ▶
              </button>
              <button className="touch-btn down" aria-label="Mover abajo" data-code="ArrowDown" {...touchHandlers}>
                ▼
              </button>
            </div>
          </div>
...
export default SerpentinaPlayer;
